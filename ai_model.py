...
def preprocess_data(df):
    ...
    return np.array(sequences), [np.array(t) for t in targets]
...
def build_lstm_model(attention=True, positional=True):
    ...
    x = Dropout(0.2)(x)
    outputs = [Dense(10, activation="softmax")(x) for _ in range(4)]
...
def train_and_save_lstm(df, lokasi):
    ...
    model.fit(X, y, epochs=30, batch_size=16, verbose=0,
              callbacks=[CSVLogger(f"training_logs/history_{lokasi.lower().replace(' ', '_')}.csv")])
