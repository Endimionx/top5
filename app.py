...
with st.sidebar:
    ...
    cari_otomatis = st.toggle("🔍 Cari Putaran Otomatis", value=True)
    ...
    if cari_otomatis and not df_all.empty:
        with st.spinner("🔍 Menganalisis putaran terbaik..."):
            best_n, best_score, _ = cari_putaran_terbaik(
                df_all, lokasi=selected_lokasi, metode=metode,
        ...
        if best_n > 0:
            putaran = best_n
            st.success(f"✅ Putaran terbaik: {putaran} (Akurasi: {best_score:.2f}%)")
        else:
            ...
            if os.path.exists(model_path):
                ...
                with col1:
                    if st.button(f"🔁 Latih Ulang {digit}"):
                        ...
                        st.success(f"✅ Model {digit} berhasil dilatih ulang.")
                with col2:
                    if st.button(f"🗑️ Hapus Model {digit}"):
                        os.remove(model_path)
                        st.warning(f"🧹 Model {digit} telah dihapus.")
            else:
...
if st.button("🔮 Prediksi"):
    if len(df) < 30:
        ...
        if result:
            ...
            with st.expander("💡 Simulasi Kombinasi 4D Confidence"):
                top_komb = (
                    kombinasi_4d(df, lokasi=selected_lokasi) if metode == "LSTM AI" else
                    kombinasi_4d_markov_hybrid(df, digit_weights={
                        "ribuan": digit_weight_input[0],
                        "ratusan": digit_weight_input[1],
                        "puluhan": digit_weight_input[2],
                        "satuan": digit_weight_input[3],
                    }) if metode == "Markov Gabungan" else []
                )
                for komb, score in top_komb:
                    st.markdown(f"**{komb}** — ⚡ Confidence: `{score:.5f}`")
