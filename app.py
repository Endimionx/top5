    ...
    with st.expander("⚙️ Manajemen Model LSTM"):
        ...
        if st.button("📚 Latih & Simpan Semua Model"):
            with st.spinner("🔄 Melatih semua model per digit..."):
                st_lottie(lottie_predicting, height=100, key="lottie_train")
...
if st.button("🔮 Prediksi"):
    if len(df) < 30:
        ...
        with st.spinner("⏳ Melakukan prediksi..."):
            st_lottie(lottie_predicting, height=100, key="lottie_prediksi")
            result = None
            if metode == "Markov":
        ...
        if result is None:
            ...
            if metode in ["LSTM AI", "Ensemble AI + Markov"]:
                with st.spinner("🔢 Menghitung kombinasi 4D terbaik..."):
                    st_lottie(lottie_predicting, height=100, key="lottie_kombinasi")
