...
from ai_model import top6_lstm, train_and_save_lstm, model_exists, low6_lstm
...
# ==================== PASARAN ====================
from lokasi_list import lokasi_list  # pisahkan jika terlalu panjang
hari_list = ["harian", "kemarin", "2hari", "3hari", "4hari", "5hari"]

selected_lokasi = st.selectbox("🌍 Pilih Pasaran", lokasi_list)
selected_hari = st.selectbox("📅 Pilih Hari", hari_list)
putaran = st.slider("🔁 Jumlah Putaran", 1, 1000, 100)
jumlah_uji = st.number_input("📊 Jumlah Data Uji Akurasi", 1, 1000, 10)
...
if metode == "LSTM AI":
    with st.expander("🛠️ Manajemen Model LSTM"):
        if st.button("📚 Latih & Simpan Model"):
            if len(df) < 20:
                st.warning("Minimal 20 data untuk melatih model.")
            else:
        ...
        if os.path.exists(model_path):
            ...
            with st.expander("📤 Upload Model LSTM"):
                uploaded_file = st.file_uploader("Upload file .h5", type=["h5"])
                if uploaded_file:
...
if st.button("🔮 Prediksi"):
    if len(df) < 11:
        ...
        if pred is None:
            ...
            if metode == "LSTM AI":
                low_pred = low6_lstm(df, selected_lokasi)
                if low_pred:
                    st.markdown("### 🚫 Angka yang Kemungkinan Tidak Keluar (Low-6)")
                    for i, label in enumerate(["Ribuan", "Ratusan", "Puluhan", "Satuan"]):
                        st.markdown(f"**{label}:** {', '.join(str(d) for d in low_pred[i])}")
